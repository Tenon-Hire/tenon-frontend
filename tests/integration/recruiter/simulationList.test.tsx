...
const listSimulationsMock = listSimulations as jest.MockedFunction<typeof listSimulations>;
...
describe("Recruiter simulations list (integration)", () => {
  ...
  it("renders simulations returned from the backend", async () => {
    ...
    await waitFor(() => expect(screen.getByText("Backend Simulation")).toBeInTheDocument());
  ...
  it("shows error message when backend call fails", async () => {
    listSimulationsMock.mockRejectedValue({ message: "Unauthorized", status: 401 });

    render(<RecruiterDashboardContent profile={null} error={null} />);

    expect(await screen.findByText(/Couldn’t load simulations/i)).toBeInTheDocument();
