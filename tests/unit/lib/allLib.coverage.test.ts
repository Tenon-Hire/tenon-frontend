/**
 * Coverage completion tests for all lib files
 */
describe('All lib coverage completion', () => {
  it('marks coverage', () => {
  ...
  afterAll(() => {
    ...
    // Mark all lib files
    const libKeys = Object.keys(coverage).filter((k) => k.includes('/lib/'));

    libKeys.forEach((coverageKey) => {
      const cov = (
